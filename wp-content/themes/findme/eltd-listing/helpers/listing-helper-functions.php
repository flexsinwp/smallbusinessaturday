...
if(!function_exists('eltd_listing_get_listing_variables')){
	/**
	 * Set JQuery eltdListingGlobalVars variable
	 */
	function eltd_listing_get_listing_variables(){
		$listing_variables = array(
			'selectedTypes' => array()
		);

		$listing_variables = apply_filters('eltd_listing_filter_listing_js_global_variables', $listing_variables);

		wp_localize_script('findme_elated_modules', 'eltdListingGlobalVars', array(
			'vars' => $listing_variables
		));
	}

	add_action('wp_enqueue_scripts', 'eltd_listing_get_listing_variables', 20);

}

if(!function_exists('eltd_listing_get_listing_types')){
	/**
	 ...
	 * return object
	 */
	function eltd_listing_get_listing_types($first_empty = false, $params = array()){
...
if(!function_exists('eltd_listing_get_listing_region')){
	/**
	 ...
	 * return object
	 */
	function eltd_listing_get_listing_region($first_empty = false){
...
if(!function_exists('eltd_listing_get_listing_cats')){
    /**
     ...
     * return object
     */
    function eltd_listing_get_listing_cats($first_empty = false){
...
if(!function_exists('eltd_listing_get_listing_types_VC_Array')){
	/**
	 * Get job listing types array prepared for Visual Composer Mapping
	 * return array
	 */
	function eltd_listing_get_listing_types_VC_Array(){

		$types_obj = eltd_listing_get_listing_types(true);
...
if(!function_exists('eltd_listing_get_listing_type_by_id')){
	/**
	 * Get job listing type
	 * param $id - job listing types id
	 * return object
	 */
	function eltd_listing_get_listing_type_by_id($id){
		$type = get_term_by('id', $id, 'job_listing_type');
		return $type;
	}
}

if(!function_exists('eltd_listing_get_listing_categories')){
	...
	function eltd_listing_get_listing_categories($params = array()){
		...
		$cats = eltd_listing_get_terms_ordered('job_listing_category', $args, $include_params, 'slug');
		...
		if(is_array($cats) && count($cats) ){
			foreach($cats as $cat){
				...
				$order_meta         = get_term_meta($cat->term_id, 'category_order', true);
				...
				$gallery_classes .= 'eltd-ls-gallery-'.esc_attr($gallery_type). ' ';
				$gallery_classes .= 'eltd-ls-gallery-'.esc_attr($gallery_size). ' ';
				...
				$cat_array[] = array(
					...
					'image_style' => eltd_listing_get_category_image_style_attr($cat->term_id),
					'icon' => eltd_listing_get_listing_category_icon_html($cat->term_id),
...
if(!function_exists('eltd_listing_get_terms_ordered')){

	function eltd_listing_get_terms_ordered( $taxonomy = '', $args = [], $term_order = '', $sort_by = 'slug' ){
...
if(!function_exists('eltd_listing_categories_VC_ARRAY')){

    function eltd_listing_categories_VC_ARRAY($first_empty = false){

		$vc_cats = array();
		$cats = eltd_listing_get_listing_categories();
...
if(!function_exists('eltd_listing_get_category_image_style_attr')){
	function eltd_listing_get_category_image_style_attr($term_id){
		$image_url_style = '';
...
if(!function_exists('eltd_listing_get_listing_type_categories')){
	...
	function eltd_listing_get_listing_type_categories($type_id){
...
if(!function_exists('eltd_listing_get_current_listing_types_categories')){
	/**
	 ...
	 * return array
	 */
	function eltd_listing_get_current_listing_types_categories($types_ids = array()){
		...
		if(is_array($types_ids) && count($types_ids)){
			foreach($types_ids as $id){
				$category_array[$id]  = eltd_listing_get_listing_type_categories($id);
			}
		}
...
if(!function_exists('eltd_listing_get_listing_type_custom_fields')){
	/**
	 * Get Listing Type custom fields
	 * param $id - Listing Type id
	 * return array
	 */
	function eltd_listing_get_listing_type_custom_fields($id){
		return get_term_meta( $id, 'listing_type_custom_fields', true );
	}
}

if(!function_exists('eltd_listing_get_listing_type_amenities')){
	/**
	 * Get Listing Type amenities
	 * param $id - Listing Type id
	 * return array
	 */
	function eltd_listing_get_listing_type_amenities($id){
		return get_term_meta( $id, 'listing_type_amenities', true );
	}
}


if(!function_exists('eltd_listing_get_listing_type_options_array')){
	/**
	 ...
	 * return array
	 */
	function eltd_listing_get_listing_type_options_array($field_obj){
		$options = array();
...
if(!function_exists('eltd_listing_get_listing_type_amenity_field_name')){
	/**
	...
	function eltd_listing_get_listing_type_amenity_field_name($type_id, $amenity){
...
if(!function_exists('eltd_listing_get_listing_type_amenity_field_name_refactored')){
    /**
     * Generate unique name for amenities
    ...
    function eltd_listing_get_listing_type_amenity_field_name_refactored($type_id, $amenity_id){
...
if(!function_exists('eltd_listing_get_listing_number_per_page')){
	/**
	 * Generate listing per page number
	 * return string
	 */
	function eltd_listing_get_listing_number_per_page(){

		$number = '';

		$eltd_option = findme_elated_options()->getOptionValue('listings_per_page');
		$wp_job_option = get_option( 'job_manager_per_page' );
		$default_option = get_option('posts_per_page');

		if($eltd_option !== ''){
			$number = esc_attr($eltd_option);
...
if(!function_exists('eltd_listing_set_listing_titles_global_var')){
	/**
	 * Localize listing titles array
	 * We use it for keyword fields autocomplete
	 */
	function eltd_listing_set_listing_titles_global_var(){

	    new Core\ListingTitleGlobalVar();

	}
	add_action('wp_footer', 'eltd_listing_set_listing_titles_global_var');
}

if(!function_exists('eltd_listing_build_query_string')){
	/**
	 * Function build query string based on params
	 * return string
	 */
	function eltd_listing_build_query_string($keyword, $type, $salary){
...
if(!function_exists('eltd_listing_get_user_address')){
	...
	function eltd_listing_get_user_address(){
...
if(!function_exists('eltd_listing_enable_categories')){
	/**
	 * Check listing categories option
	 * return bool
	 */
	function eltd_listing_enable_categories(){
...
if(!function_exists('eltd_listing_get_listing_types_by_listing_id')){
	/**
	 * Get Listing Types by listing id
	 * params $id - listing id
	 * return array
	 */
	function eltd_listing_get_listing_types_by_listing_id($id){
...
if(!function_exists('eltd_listing_get_listing_categories_by_listing_id')){
	/**
	 * Get Listing Categories by listing id
	 * params $id - listing id
	 * return array
	 */
	function eltd_listing_get_listing_categories_by_listing_id($id){
		...
		if(is_array($types) && count($types)){
			foreach($types as $type){
				$cats[]  = array(
					...
					'icon_html' => eltd_listing_get_listing_category_icon_html($type->term_id)
				);
			}
		}

		if(count($cats)){
			$html .= '<div class="eltd-listing-cat-wrapper">';
			foreach($cats as $cat){
				$html .= '<a href="'.esc_url($cat['link']).'">';
				$html .= '<span class="eltd-listing-cat-icon">';
				$html .= eltd_listing_get_listing_category_icon_html($cat['id']);
				$html .= '</span>';
				$html .= '<span class="eltd-listing-cat-name">'.esc_attr($cat['name']).'</span>';
				$html .= '</a>';
			}
			$html .= '</div>';
		}
...
if(!function_exists('eltd_listing_get_listing_category_icon_html')){
	/**
	 * Get Listing Category Icon Html
	 * params $id - listing id
	 * return string
	 */
	function eltd_listing_get_listing_category_icon_html($id){
		...
		if(!empty($custom_icon)){
			$classes[] = 'eltd-custom-icon';
			$icon_html = '<img src="'.$custom_icon.'" alt="'.esc_attr__('Category Custom Icon', 'eltd-listing').'"/>';
		}
		elseif($icon_pack !== ''){
			$classes[] = 'eltd-regular-icon';
			$param = findme_elated_icon_collections()->getIconCollectionParamNameByKey($icon_pack);
			$icon = get_term_meta( $id, $param, true );

			if($icon !== ''){
				$icon_html = findme_elated_icon_collections()->renderIcon( $icon, $icon_pack );
			}
		}
...
if(!function_exists('eltd_listing_get_free_package')){
	/**
	 * Get Free Package
	 * return array
	 */
	function eltd_listing_get_free_package(){

		$package = array();
		$package_obj = false;

		$package_args = eltd_listing_get_free_package_params();
...
if(!function_exists('eltd_listing_get_free_package_params')){
	/**
	 * Get Free Package Params Array
	 * return array
	 */
	function eltd_listing_get_free_package_params(){
		$params  =  array(
			'post_type'        => 'product',
			'posts_per_page'   => -1,
			'suppress_filters' => 0,
...
if(!function_exists('eltd_listing_get_listing_rating_html')){
	/**
	 ...
	 * @return string
	 */
	function eltd_listing_get_listing_rating_html($id){
...
if(!function_exists('eltd_listing_get_listing_price_html')){
	/**
	 ...
	 * @return string
	 */
	function eltd_listing_get_listing_price_html($id){
		...
		if(($price && $price !== '') || ($disc_price && $disc_price !== '')){
			...
			$price_html .= '<div class="eltd-ls-price-holder">';
			if($price && $price !== ''){
				$price_html .= '<span class="eltd-ls-price-amount">';
				$price_html .= esc_attr('$').esc_attr($price);
				$price_html .= '</span>';
			}
			if($disc_price && $disc_price !== ''){
				$price_html .= '<span class="eltd-ls-disc-price-amount">';
...
if(!function_exists('eltd_listing_get_listing_social_network_array')){
	/**
	 * Function generate social network array
	 *
	 * @return array
	 */
	function eltd_listing_get_listing_social_network_array(){

		$social_networks =  array(
			'facebook' => esc_html__('Facebook', 'eltd-listing'),
			'twitter' => esc_html__('Twitter', 'eltd-listing'),
			'instagram' => esc_html__('Instagram', 'eltd-listing'),
			'pinterest' => esc_html__('Pinterest', 'eltd-listing'),
			'soundcloud' => esc_html__('Sound Cloud', 'eltd-listing'),
			'vimeo' => esc_html__('Vimeo', 'eltd-listing'),
			'youtube' => esc_html__('Youtube', 'eltd-listing'),
			'skype' => esc_html__('Skype', 'eltd-listing')
		);
		...
		foreach ($social_networks as $net_id => $net_name){
			...
			$return_array[$net_id]['label'] = esc_html__('Enter ','eltd-listing').esc_attr($net_name). esc_html__(' Profile Url', 'eltd-listing');
...
if(!function_exists('eltd_listing_comment_relative_date')){
	/**
	 * Function generate relative date for listing comments
	 *
	 */
	function eltd_listing_comment_relative_date() {
		return human_time_diff( get_the_time('U'), current_time( 'timestamp' ) ) . esc_html__(' ago', 'eltd-listing');
	}
	add_filter( 'get_comment_date', 'eltd_listing_comment_relative_date' );
}