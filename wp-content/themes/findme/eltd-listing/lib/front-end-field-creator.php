<?php
namespace ElatedListing\Lib\Front;
class ListingTypeFieldCreator{
	...
	private function getTypeCategories(){
		return eltd_listing_get_listing_type_categories($this->id);
	}

	private function getTypeCustomFields(){
		return eltd_listing_get_listing_type_custom_fields($this->id);
	}

	private function getTypeAmenities(){
		return eltd_listing_get_listing_type_amenities($this->id);
	}

	private function getTypeTitle(){
		$type = eltd_listing_get_listing_type_by_id($this->id);
		return esc_html__('Listing Type', 'eltd-listing').' "'.esc_attr($type->name).'"';
	}
	...
	public function renderListingFormFields(){?>
		...
		<div id="<?php echo esc_attr($this->id) ?>" class="eltd-ls-type-field-wrapper" data-ls-type-id="eltd-ls-type-field-wrapper-<?php echo esc_attr($this->id) ?>">
	...
	private function renderCategoryField($html_type = ''){
		...
		if(is_array($this->categories) && count($this->categories)){ ?>
			...
			<h4 class="eltd-listing-field-holder-title">
				<?php esc_html_e('Categories', 'eltd-listing') ; ?>
			</h4>

			<?php
			$saved_value = '';
			if($this->post_id !== ''){
				$saved_value = get_post_meta($this->post_id, 'eltd_listing_type_categories', true);
			}
	...
	private function renderCustomFields($html_type = ''){
		...
		if(is_array($this->custom_fields) && count($this->custom_fields)){ ?>
			<h4 class="eltd-listing-field-holder-title">
				<?php esc_html_e('Custom Fields', 'eltd-listing') ; ?>
			</h4>
	...
	private function renderAmenities($html_type = ''){
		...
		if(is_array($this->amenities) && count($this->amenities)){
			...
			if($html_type !=='single'){ ?>
                ...
                <h5 class="eltd-listing-field-holder-title">
					<?php esc_html_e('Filter by Amenities', 'eltd-listing') ; ?>
				</h5>

				<div class="eltd-listing-amenities-wrapper eltd-<?php echo esc_attr($column_number)?>-columns clearfix">

                    <?php foreach($this->amenities as $key => $amenity){
						$counter++;
						//for each 4n+1 item open inner div
						if($counter % 4 === 1){?>
							<div class="eltd-listing-amenities-wrapper-inner">
						<?php }
						$amenity_field_name = eltd_listing_get_listing_type_amenity_field_name_refactored($this->id, $key);
	...
	private function renderSubmitButton(){
		...
		echo findme_elated_get_button_html(array(
			'size'     => 'medium',
			'type'     => 'solid',
			'custom_class' => 'eltd-adv-search-submit',
...
class FrontFieldCheckBoxGroup{
		...
		<fieldset class="fieldset-<?php echo esc_attr($this->name); ?>">
			...
			<div class="field">
				...
				foreach($this->options as $option_key => $option_label){
					...
					<div class="checkbox-inline eltd-ls-checkbox-field">
	...
	private function renderAdvSearchHtml(){	?>
		...
		<div class="eltd-ls-adv-search-field-wrapper">
			<?php foreach($this->options as $option_key => $option_label){?>

				<div class="eltd-ls-adv-search-field checkbox">

					<input type="checkbox"  id="<?php echo esc_attr($option_key); ?>" class="eltd-ls-adv-search-input" value="<?php echo esc_attr($option_key); ?>" name="<?php echo esc_attr($this->name); ?>"/>
					<label>
	...
	private function renderSingleListingHtml(){
	    ...
	    if(is_array($this->value) && count($this->value)){?>
			...
			<div  class="eltd-listing-single-field">
...
class FrontFieldSelect{
	...
	private function renderListingFieldHtml(){ ?>
		...
		<fieldset class="fieldset-<?php echo esc_attr($this->name); ?>">
            ...
            <label>
                ...
                <?php echo esc_attr($this->label); ?>
				<small>
                    <?php esc_html_e('(optional)','eltd-listing') ?>
                </small>
	...
	private function renderAdvSearchHtml(){ ?>
		...
		<div class="eltd-ls-adv-search-field select">

            <label>
				<?php echo esc_attr($this->label); ?>
			</label>

			<select class="eltd-ls-adv-search-input" name="<?php echo esc_attr($this->name); ?>">
				<option value=""></option>
...
class FrontFieldText{
	...
	private function renderAdvSearchHtml(){ ?>
		...
		<div class="eltd-ls-adv-search-field">

			<label>
				<?php echo esc_attr($this->label); ?>
			</label>

			<input type="text" name="<?php echo esc_attr($this->name); ?>" class="eltd-ls-adv-search-input"/>
	...
	private function renderSingleListingHtml(){
	    ...
	    if($this->value !== ''){ ?>
			...
			<div  class="eltd-listing-single-field text">
...
class FrontFieldTextArea{
	...
	private function renderAdvSearchHtml(){ ?>
		...
		<div class="eltd-ls-adv-search-field textarea">

			<label>
				<?php echo esc_attr($this->label); ?>
			</label>

			<textarea name="<?php echo esc_attr($this->name); ?>" rows="5" class="eltd-ls-adv-search-input"></textarea>
		</div>
	<?php }
...
class FrontFieldCheckBox{
	...
	private function renderListingFieldHtml(){
		...
		<fieldset class="fieldset-<?php echo esc_attr($this->name); ?>">
            ...
            <div class="field eltd-ls-checkbox-field">
	...
	private function renderAdvSearchHtml(){
		...
		if($this->amenity_flag){
			$amenity_class = 'eltd-amenity-field';
		}
		?>

		<div class="eltd-ls-adv-search-field checkbox">
			<input type="checkbox" class="eltd-ls-adv-search-input <?php echo esc_attr($amenity_class); ?>" name="<?php echo esc_attr($this->name); ?>"/>
	...
	private function renderArchiveSearchHtml(){
		...
		<div class="eltd-listing-type-amenity-field">
			<input type="checkbox" class="eltd-listing-type-amenity-field <?php echo esc_attr($amenity_class); ?>" name="<?php echo esc_attr($this->name); ?>"/>
	...
	private function renderSingleListingHtml(){
		 if($this->value === '1'){ ?>
			...
			<div  class="eltd-listing-single-field">
				<div class="eltd-listing-single-field-inner eltd-ls-icon">
					<?php
						echo findme_elated_icon_collections()->renderIcon( $this->icon, $this->icon_pack );
					?>
				</div>
				<div class="eltd-listing-single-field-inner eltd-ls-text">
					<?php echo esc_attr($this->label); ?>
				</div>
			</div>
