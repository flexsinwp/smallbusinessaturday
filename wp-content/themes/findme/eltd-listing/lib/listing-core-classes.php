<?php
namespace ElatedListing\Lib\Core;
class ListingQuery{
	...
	public function __construct($listing_type_id = '', $post_number = '-1', $category_array = array(), $meta_query_flag = false, $checkbox_meta_params = array(), $default_meta_params = array(), $next_page = '', $user_id = '', $post_status = 'publish', $keyword = '', $post__in = array(), $post__not_in = array(), $tag = '', $location = '', $tax_query = array(), $location_object = array(), $sort_param = '') {
	...
	private function generateQueryArray(){
		...
		if(isset($this->listing_type_id) && $this->listing_type_id !== '' && $this->listing_type_id !== 'all'){
			...
			if($order_by !== ''){
				switch ($order_by){
					...
					case 'rating':
						...
						$this->query_array['meta_key'] = 'eltd_post_rating_value';
						break;
				}
			}
	...
	private function setQueryResults(){
		...
		if(isset($this->location_object['dist']) && isset($this->location_object['lat']) && isset($this->location_object['long'])){
			...
			$geo_location_answer  = eltd_listing_check_distance($lat, $long, $dist, $posts_to_check);
...
class ListingRating{
	...
	public function setAverageRating(){
        ...
        $rating_criteria = eltd_rating_criteria('job_listing');
	...
	private function getRateNumber(){
		return get_post_meta($this->post_id, 'eltd_post_rating_number', true);
	}

	private function setRateNumber($count){
		update_post_meta($this->post_id, 'eltd_post_rating_number', $count);
	}
	...
	private function getRateCount(){
		return get_post_meta($this->post_id, 'eltd_post_rating_value', true);
	}

	private function setRateCount($value){
		update_post_meta($this->post_id, 'eltd_post_rating_value', $value);
	}
		...
		?>
			<div class="eltd-listing-rating-holder">
				...
				<div class="eltd-listing-rating-stars-holder">
					<span class="eltd-rating-stars" <?php echo findme_elated_get_inline_style($style) ?>></span>
				</div>
...
class ListingViews{
	...
	public function __construct($post_id, $user_address = '' ) {
		...
		$this->cookie_name = 'eltd_listing_single_id_'.$this->post_id;
	...
	public function getViewCount(){
		return get_post_meta($this->post_id, 'eltd_post_view_count', true);
	}

	private function setViewCount($value){
		update_post_meta($this->post_id, 'eltd_post_view_count', $value);
	}
...
class ListingArticle{
		...
		if(is_array($taxes) && count($taxes)){
			foreach($taxes as $tax){
				...
				$tax_array[]  = array(
					...
					'icon_html' => eltd_listing_get_listing_category_icon_html($tax->term_id)
				);
	...
	public function getTaxHtml($taxonomy, $custom_css_class = '', $show_title = false){
		...
		if(count($taxes)){
			$html .= '<div class="eltd-tax-wrapper '.esc_attr($custom_css_class).'">';
			foreach($taxes as $tax){
				...
				if($show_title){
				    $html .= '<span class="eltd-tax-title">';
	...
	public function getAddressIconHtml(){
		...
		$params_address = eltd_listing_get_address_params($this->post_id);
	...
	public function getPriceHtml(){
		...
		if(($price && $price !== '') || ($disc_price && $disc_price !== '')){
			...
			if($disc_price && $disc_price !== $price ){
	...
	public function getActualPriceHtml(){
		...
		if($disc_price && $disc_price !== ''){
			...
			$price_html .= '<div class="eltd-ls-actual-price-holder">';
...
class ListingTitleGlobalVar{
	...
	$query_results = eltd_listing_get_listing_query_results($params);
	...
	$listing_posts = apply_filters('eltd_listing_filter_js_listing_variables', $listing_posts);

	wp_localize_script('findme_elated_modules', 'eltdListingTitles', array(
