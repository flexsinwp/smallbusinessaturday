<?php
namespace ElatedListing\Archive;
...
class ArchiveGlobalVar{
	...
	public function setArchiveGlobalVars(){
		...
		$archive_global_var = apply_filters('eltd_listing_filter_listing_archive_var', $archive_global_var);

		wp_localize_script('findme_elated_modules', 'eltdListingArchiveVar', array(
	...
	public function registerSingleTemplate($single) {
		...
		if($post->post_type == $this->base) {
			...
			if(!file_exists(get_template_directory().'/single-'.$this->base.'.php')) {
				return ELATED_LISTING_ABS_PATH.'/templates/single-'.$this->base.'.php';
			}
		}
		...
		if($post && $post !== null && $post->post_type == $this->base) {
			...
			foreach($this->taxBaseArray as $tax){
				if(!file_exists(get_template_directory().'/archive-'.$tax.'.php')) {
					return ELATED_LISTING_ABS_PATH.'/templates/archive-'.$tax.'.php';
				}
			}
		}
