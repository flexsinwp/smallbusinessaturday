 ...
 */
if(!class_exists('FindmeElatedVerticalCompactNavigationWalker')) {
	class FindmeElatedVerticalCompactNavigationWalker extends Walker_Nav_Menu {
		...
		function start_lvl(&$output, $depth = 0, $args = array()) {
			...
			if($this->background_image !== '') {
				...
				$background_image_class = ' eltd-dropdown-with-background-image';
			}

			$indent = str_repeat("\t", $depth);
			if($depth == 0) {
				$out_div = '<div class="second '.$background_image_class.'" '.findme_elated_get_inline_style($menu_image_background).'><div class="inner">';
			} else {
				$out_div = '';
			}
		...
		function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
			...
			if(!empty($item->icon)) {
				$class_names .= ' eltd-menu-item-with-icon';
			}

			if(!empty($item->description)) {
				$class_names .= ' eltd-menu-item-with-description';
			}
