...
if(!function_exists('findme_elated_get_title')) {
    /**
     *
     ...
     */
    function findme_elated_get_title($module_title = false, $module = '', $layout = '') {
        $id = findme_elated_get_page_id();

        extract(findme_elated_title_area_height());
        extract(findme_elated_title_area_background());
	    
        //check if title area is visible on page first, then in the options, modules have priority
        $show_title_area = findme_elated_get_meta_field_intersect('show_title_area') == 'yes' ? true : false;
        if(!empty($module)) {
            $show_title_area_meta = findme_elated_get_meta_field_intersect('show_title_area_'.$module);
            if(!empty($show_title_area_meta)) {
                $show_title_area = $show_title_area_meta == 'yes' ? true : false;
            }
        }

        if(findme_elated_is_woocommerce_installed() && is_singular('product')) {
            $woo_title_meta = get_post_meta(get_the_ID(), 'eltd_show_title_area_woo_meta', true);
            if(empty($woo_title_meta)) {
                $woo_title_main = findme_elated_options()->getOptionValue('show_title_area_woo');
        ...
        if($show_title_area) {
            ...
            $type = findme_elated_get_meta_field_intersect('title_area_type');

            //check if breadcrumbs are enabled on page first, then in options
            $enable_breadcrumbs_meta = findme_elated_get_meta_field_intersect('title_area_enable_breadcrumbs');
            $enable_breadcrumbs = $enable_breadcrumbs_meta == 'yes' ? true : false;

            //check title tag value
            $title_tag_meta = findme_elated_get_meta_field_intersect('title_area_title_tag');
            $title_tag = !empty($title_tag_meta) ? $title_tag_meta : 'h1';

            //check if title color is set on page
            $title_color_meta = get_post_meta($id, 'eltd_title_text_color_meta', true);
            $title_color = !empty($title_color_meta) ? 'color:' . esc_attr($title_color_meta) . ';' : '';

            //check if subtitle color is set on page
            $subtitle_color_meta = get_post_meta($id, 'eltd_subtitle_color_meta', true);
            ...
            $parameters = apply_filters('findme_elated_title_area_height_params', $parameters);

            if ($module_title && $module != '') {
                findme_elated_get_module_template_part('templates/titles/title', $module, $layout, $parameters);
            } else {
                findme_elated_get_module_template_part('templates/title', 'title', $layout, $parameters);
            }
        }
    }
}

if(!function_exists('findme_elated_get_title_text')) {
    /**
     * Function that returns current page title text. Defines findme_elated_title_text filter
     ...
     */
    function findme_elated_get_title_text() {
        ...
        if (is_tag()) {
            ...
            $title = single_term_title("", false).esc_html__(' Tag', 'findme');
        }
        ...
        elseif (is_author()) {
            ...
            $title = esc_html__('Author:', 'findme') . " " . get_the_author();
        }
        ...
        elseif (is_search()) {
            ...
            $title = esc_html__('Search results for: ', 'findme') . get_search_query();
        }

        //is single post page
        elseif (is_singular('post')) {
	        //get title for single post page
	        $title = findme_elated_options()->getOptionValue('blog_single_title_in_title_area') == 'yes' ? get_the_title($id) : esc_html__('FindMe', 'findme');
        }

        //is current page 404?
        elseif (is_404()) {
            //is 404 title text set in theme options?
	        $title_404 = findme_elated_options()->getOptionValue('404_title');
	        $title = !empty($title_404) ? $title_404 : esc_html__('404 - Page not found', 'findme');
        }

        //is WooCommerce installed and is shop or single product page?
        elseif(findme_elated_is_woocommerce_installed() && (findme_elated_is_woocommerce_shop() || is_singular('product'))) {
        ...
        elseif(findme_elated_is_woocommerce_installed() && (is_product_category() || is_product_tag())) {
            global $wp_query;
        ...
        elseif (is_archive()) {
            $title = esc_html__('Archive','findme');
        }
        ...
        $title = apply_filters('findme_elated_title_text', $title);

        return $title;
    }
}

if(!function_exists('findme_elated_title_text')) {
    /**
     * Function that echoes title text.
     *
     * @see findme_elated_get_title_text()
     */
    function findme_elated_title_text() {
        echo findme_elated_get_title_text();
    }
}

if(!function_exists('findme_elated_subtitle_text')) {
    /**
     * Function that echoes subtitle text.
     */
    function findme_elated_subtitle_text() {
        $id = findme_elated_get_page_id();
        $subtitle 	= '';
...
if(!function_exists('findme_elated_custom_breadcrumbs')) {
    /**
     * Function that renders breadcrumbs
     */
    function findme_elated_custom_breadcrumbs() {
        global $post;

        $output = "";
        $homeLink = esc_url(home_url('/'));
        $pageid = findme_elated_get_page_id();
        $bread_style = "";
        ...
        $delimiter = '<span class="eltd-delimiter" '.findme_elated_get_inline_style($bread_style).'>&nbsp; / &nbsp;</span>'; // delimiter between crumbs
        $home = esc_html__('Home', 'findme'); // text for the 'Home' link
        $showCurrent = 1; // 1 - show current post/page title in breadcrumbs, 0 - don't show
        $before = '<span class="eltd-current" '.findme_elated_get_inline_style($bread_style).'>'; // tag before the current crumb
        $after = '</span>'; // tag after the current crumb

        if (is_home() && !is_front_page()) {
            $output = '<div class="eltd-breadcrumbs"><div class="eltd-breadcrumbs-inner" itemprop="breadcrumb"><a itemprop="url" '.findme_elated_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter . ' <a itemprop="url" '.findme_elated_get_inline_style($bread_style).' href="' . $homeLink . '">'. get_the_title($pageid) .'</a></div></div>';

        } elseif(is_home()) {
            $output = '<div class="eltd-breadcrumbs"><div class="eltd-breadcrumbs-inner" itemprop="breadcrumb">'.$before.$home.$after.'</div></div>';
        }

        elseif(is_front_page()) {
            if ($showOnHome == 1) $output = '<div class="eltd-breadcrumbs"><div class="eltd-breadcrumbs-inner" itemprop="breadcrumb"><a itemprop="url" '.findme_elated_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a></div></div>';
        }

        else {

            $output .= '<div class="eltd-breadcrumbs"><div class="eltd-breadcrumbs-inner" itemprop="breadcrumb"><a itemprop="url" '.findme_elated_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter;

            if ( is_category() || (findme_elated_is_woocommerce_installed() && findme_elated_is_product_category())) {
                ...
                $output .= $before . esc_html__('Search results for ', 'findme') . '"' . get_search_query() . '"' . $after;

            } elseif ( is_day() ) {
                $output .= '<a itemprop="url" '.findme_elated_get_inline_style($bread_style).' href="' . get_year_link(get_the_time('Y')) . '">' . get_the_time('Y') . '</a>' . $delimiter;
                $output .= '<a itemprop="url" '.findme_elated_get_inline_style($bread_style).' href="' . get_month_link(get_the_time('Y'),get_the_time('m')) . '">' . get_the_time('F') . '</a>' . $delimiter;
            ...
            } elseif ( findme_elated_is_woocommerce_installed() && is_singular('product') ){
	            $product = wc_get_product(get_the_ID());
	            
                if(get_option('woocommerce_shop_page_id')){
                    $output .= '<a itemprop="url" '. findme_elated_get_inline_style($bread_style) .' href="' . get_permalink(get_option('woocommerce_shop_page_id')) . '">' . get_the_title(get_option('woocommerce_shop_page_id')) . '</a>' . $delimiter;
                }
                ...
                if($post->post_parent) {
                    ...
                    $output .= '<a itemprop="url" '.findme_elated_get_inline_style($bread_style).' href="' . get_permalink($parent) . '">' . $parent->post_title . '</a>';
                ...
                if($post->post_parent) {
                    ...
                    foreach($parent_ids as $parent_id) {
                        $output .= '<a itemprop="url" '.findme_elated_get_inline_style($bread_style).' href="' . get_the_permalink($parent_id) . '">' . get_the_title($parent_id) . '</a>' . $delimiter;
                    }
                }
                ...
                $output .= $before . esc_html__('Posts tagged ','findme') .'"' . single_tag_title('', false) . '"' . $after;

            } elseif ( is_author() ) {
                $author_id = get_query_var('author');
                $output .= $before . esc_html__('Articles posted by ','findme') . get_the_author_meta('display_name', $author_id) . $after;

            } elseif ( is_404() ) {
                $output .= $before . esc_html__('Error 404','findme') . $after;

            } elseif(findme_elated_is_woocommerce_installed() && findme_elated_is_woocommerce_shop()){
                $shop_id = get_option('woocommerce_shop_page_id');
            ...
            if ( get_query_var('paged') ) {
                ...
                $output .= $before . " (" . esc_html__('Page', 'findme') . ' ' . get_query_var('paged') . ")" . $after;
            }
...
if(!function_exists('findme_elated_get_title_area_height_default_value')) {
    /**
     * Function that returns title height
     **/
    function findme_elated_get_title_area_height_default_value() {
        $title_height = 200;

        return apply_filters('findme_elated_title_area_height_default_value', $title_height);
    }
}

if(!function_exists('findme_elated_get_title_area_height')) {
    /**
     * Function that returns title height
     **/
    function findme_elated_get_title_area_height() {
		$title_height_meta = findme_elated_get_meta_field_intersect('title_area_height');
	
	    $title_height = !empty($title_height_meta) ? $title_height_meta : findme_elated_get_title_area_height_default_value();

        return apply_filters('findme_elated_title_area_height', $title_height);
    }
}

if(!function_exists('findme_elated_get_title_content_padding')) {
    /**
     * Function that returns title content pading
     **/
    function findme_elated_get_title_content_padding() {
        return apply_filters('findme_elated_title_content_padding', 0);
    }
}

if(!function_exists('findme_elated_title_area_height')) {
    /**
     * Function that returns title height and padding to be applied in template
     **/

    function findme_elated_title_area_height() {
        $title_height_and_padding = array();
        $title_height          = findme_elated_get_title_area_height();
        $header_height_padding = findme_elated_get_title_content_padding();
        $title_holder_height = '';
	    ...
	    $title_vertical_alignment = findme_elated_get_meta_field_intersect('title_area_vertical_alignment');
...
if(!function_exists('findme_elated_title_area_background')) {
    /**
     ...
     * @return array
     **/
    function findme_elated_title_area_background() {
        $id = findme_elated_get_page_id();
        $show_title_img = true;
	    ...
	    $background_color = findme_elated_get_meta_field_intersect('title_area_background_color');

        //check if background image is set on page or in options
        $background_image = findme_elated_get_meta_field_intersect('title_area_background_image');
