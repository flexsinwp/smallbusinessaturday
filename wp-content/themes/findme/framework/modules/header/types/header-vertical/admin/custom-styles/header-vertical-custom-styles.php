...
if ( ! function_exists( 'findme_elated_vertical_menu_styles' ) ) {
	function findme_elated_vertical_menu_styles() {
		...
		$vertical_background_color = findme_elated_options()->getOptionValue( 'vertical_header_background_color' );
		$vertical_background_image = findme_elated_options()->getOptionValue( 'vertical_header_background_image' );
		$vertical_shadow_enabled   = findme_elated_options()->getOptionValue( 'vertical_header_shadow' );
		$vertical_border_enabled   = findme_elated_options()->getOptionValue( 'vertical_header_border' );
		...
		if ( $vertical_border_enabled === 'yes' ) {
			$header_border_color = findme_elated_options()->getOptionValue( 'vertical_header_border_color' );
		...
		echo findme_elated_dynamic_css( $vertical_header_selectors, $vertical_header_styles );
	}
	
	add_action( 'findme_elated_style_dynamic', 'findme_elated_vertical_menu_styles' );
}

if ( ! function_exists( 'findme_elated_vertical_main_menu_styles' ) ) {
	/**
	 * Generates styles for vertical main main menu
	 */
	function findme_elated_vertical_main_menu_styles() {
		$menu_holder_styles = array();
		
		$menu_top_margin    = findme_elated_options()->getOptionValue( 'vertical_menu_top_margin' );
		$menu_bottom_margin = findme_elated_options()->getOptionValue( 'vertical_menu_bottom_margin' );
		
		if ( ! empty( $menu_top_margin ) ) {
			$menu_holder_styles['margin-top'] = findme_elated_filter_px( $menu_top_margin ) . 'px';
		}
		if ( ! empty( $menu_bottom_margin ) ) {
			$menu_holder_styles['margin-bottom'] = findme_elated_filter_px( $menu_bottom_margin ) . 'px';
		}
		...
		echo findme_elated_dynamic_css( $menu_holder_selector, $menu_holder_styles );
		
		// vertical menu 1st level style
		
		$first_level_styles = findme_elated_get_typography_styles( 'vertical_menu_1st' );
		
		$first_level_selector = array(
			'.eltd-header-vertical .eltd-vertical-menu > ul > li > a'
		);
		
		echo findme_elated_dynamic_css( $first_level_selector, $first_level_styles );
		
		$first_level_hover_styles = array();
		
		$first_level_hover_color = findme_elated_options()->getOptionValue( 'vertical_menu_1st_hover_color' );
		...
		echo findme_elated_dynamic_css( $first_level_hover_selector, $first_level_hover_styles );
		
		// vertical menu 2nd level style
		
		$second_level_styles = findme_elated_get_typography_styles( 'vertical_menu_2nd' );
		
		$second_level_selector = array(
			'.eltd-header-vertical .eltd-vertical-menu .second .inner > ul > li > a'
		);
		
		echo findme_elated_dynamic_css( $second_level_selector, $second_level_styles );
		
		$second_level_hover_styles = array();
		
		$second_level_hover_color = findme_elated_options()->getOptionValue( 'vertical_menu_2nd_hover_color' );
		...
		echo findme_elated_dynamic_css( $second_level_hover_selector, $second_level_hover_styles );
		
		// vertical menu 3rd level style
		
		$third_level_styles = findme_elated_get_typography_styles( 'vertical_menu_3rd' );
		
		$third_level_selector = array(
			'.eltd-header-vertical .eltd-vertical-menu .second .inner ul li ul li a'
		);
		
		echo findme_elated_dynamic_css( $third_level_selector, $third_level_styles );
		
		
		$third_level_hover_styles = array();
		
		$third_level_hover_color = findme_elated_options()->getOptionValue( 'vertical_menu_3rd_hover_color' );
		...
		echo findme_elated_dynamic_css( $third_level_hover_selector, $third_level_hover_styles );
	}
	
	add_action( 'findme_elated_style_dynamic', 'findme_elated_vertical_main_menu_styles' );
}