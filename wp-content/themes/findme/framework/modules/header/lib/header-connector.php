<?php
namespace FindmeElated\Modules\Header\Lib;
/**
...
class HeaderConnector {
    ...
    public function connect($config = array()) {
        do_action('findme_elated_pre_header_connect');
        ...
        if(!empty($config['affect_content'])) {
            add_filter('findme_elated_content_elem_style_attr', array($this, 'contentMarginFilter'));
        }

        if(!empty($config['affect_title'])) {
            add_filter('findme_elated_title_content_padding', array($this, 'titlePaddingFilter'));
        }

        do_action('findme_elated_after_header_connect');
    }
     ...
     *
     * Hooks to findme_elated_title_content_padding filter
     *
     * @return int
     */
