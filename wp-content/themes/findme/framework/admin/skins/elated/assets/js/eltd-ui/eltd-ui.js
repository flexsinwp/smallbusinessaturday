(function($){
            ...
            frame: function() {
                ...
                this._frame.on('update', function() {
                    ...
                    jQuery.ajax({
                        type: "post",
                        url: ajaxurl,
                        data: "action=findme_elated_gallery_upload_get_images&ids=" + ids,
    ...
    function eltdInitAjaxForm() {
        ...
        $(document).delegate(".eltd_ajax_form", "submit", function (a) {
            var b = $(this);
            var c = {
                action: "findme_elated_save_options"
            };
            jQuery.ajax({
                url: ajaxurl,
                cache: !1,
            ...
            eltdImportBtn.on('click', function(e) {
                ...
                if (confirm(confirmMessage)) {
                    ...
                    if(import_opt == 'content'){
                        for( var i=1; i < 10; i++ ){
                            var str;
                            if (i < 10) str = 'findme_content_0'+i+'.xml';
                            else str = 'findme_content_'+i+'.xml';
                            jQuery.ajax({
                                type: 'POST',
                                url: ajaxurl,
                                data: {
