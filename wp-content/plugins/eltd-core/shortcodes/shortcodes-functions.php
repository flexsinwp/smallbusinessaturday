...
if ( ! function_exists( 'eltd_core_add_admin_shortcodes_styles' ) ) {
	...
	function eltd_core_add_admin_shortcodes_styles() {
		...
		wp_enqueue_style( 'eltd_core_vc_shortcodes', ELATED_CORE_ASSETS_URL_PATH . '/css/admin/eltd-vc-shortcodes.css' );
	}
	
	add_action( 'findme_elated_admin_scripts_init', 'eltd_core_add_admin_shortcodes_styles' );
}
...
if ( ! function_exists( 'eltd_core_add_admin_shortcodes_custom_styles' ) ) {
	...
	function eltd_core_add_admin_shortcodes_custom_styles() {
		...
		if ( ! empty( $style ) ) {
			wp_add_inline_style( 'eltd_core_vc_shortcodes', $style );
		}
	}
	
	add_action( 'findme_elated_admin_scripts_init', 'eltd_core_add_admin_shortcodes_custom_styles' );
}