...
class MasonryGallery implements Lib\ShortcodeInterface {
    ...
    public function __construct() {
        $this->base = 'eltd_masonry_gallery';

        add_action('vc_before_init', array($this, 'vcMap'));
	
	    //Masonry Gallery category filter
	    add_filter( 'vc_autocomplete_eltd_masonry_gallery_category_callback', array( &$this, 'masonryGalleryCategoryAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array
	
	    //Masonry Gallery category render
	    add_filter( 'vc_autocomplete_eltd_masonry_gallery_category_render', array( &$this, 'masonryGalleryCategoryAutocompleteRender', ), 10, 1 ); // Get suggestion(find). Must return an array
    }
    ...
    public function render($atts, $content = null) {
	    ...
	    if(!empty($space_between_items)) {
		    $holder_classes = 'eltd-mg-'.$space_between_items.'-space';
	    }
        
        $query = new \WP_Query( $query_args );

        $html .= '<div class="eltd-masonry-gallery-holder '.esc_attr($holder_classes).'">';
	        $html .= '<div class="eltd-mg-inner">';
	            $html .= '<div class="eltd-mg-grid-sizer"></div>';
		        $html .= '<div class="eltd-mg-grid-gutter"></div>';
	
		        if ($query->have_posts()) :
		            while ( $query->have_posts() ) : $query->the_post();
		
						if (get_post_meta(get_the_ID(), 'eltd_masonry_gallery_item_type', true) !== '') {
							$type = get_post_meta(get_the_ID(), 'eltd_masonry_gallery_item_type', true);
						} else {
							$type = 'standard';
						}
			            ...
			            if (get_post_meta(get_the_ID(), 'eltd_masonry_gallery_item_title_tag', true) !== '') {
				            $params['item_title_tag'] = get_post_meta(get_the_ID(), 'eltd_masonry_gallery_item_title_tag', true);
			            } else {
				            $params['item_title_tag'] = 'h4';
			            }
			            if (get_post_meta(get_the_ID(), 'eltd_masonry_gallery_item_text', true) !== '') {
				            $params['item_text'] = get_post_meta(get_the_ID(), 'eltd_masonry_gallery_item_text', true);
			            }
		                if (get_post_meta(get_the_ID(), 'eltd_masonry_gallery_item_link', true) !== '') {
							$params['item_link'] = get_post_meta(get_the_ID(), 'eltd_masonry_gallery_item_link', true);
		                }
		                if (get_post_meta(get_the_ID(), 'eltd_masonry_gallery_item_link_target', true) !== '') {
							$params['item_link_target'] = get_post_meta(get_the_ID(), 'eltd_masonry_gallery_item_link_target', true);
		                }
						if (get_post_meta(get_the_ID(), 'eltd_masonry_gallery_button_label', true) !== '') {
							$params['item_button_label'] = get_post_meta(get_the_ID(), 'eltd_masonry_gallery_button_label', true);
		                }
						...
						$html .= eltd_core_get_cpt_shortcode_module_template_part('masonry-gallery', 'masonry-gallery-'. $type . '-template', '', $params);
	...
	private function getItemClasses(){
		$classes = array('eltd-mg-item');
		
		if (get_post_meta(get_the_ID(), 'eltd_masonry_gallery_item_type', true) !== '') {
			$classes[] = 'eltd-mg-' . get_post_meta(get_the_ID(), 'eltd_masonry_gallery_item_type', true);
		}

		if (get_post_meta(get_the_ID(), 'eltd_masonry_gallery_item_size', true) !== '') {
			$classes[] = 'eltd-mg-' . get_post_meta(get_the_ID(), 'eltd_masonry_gallery_item_size', true);
		}
		
		if (get_post_meta(get_the_ID(), 'eltd_masonry_gallery_simple_content_background_skin', true) !== '') {
			$classes[] = 'eltd-mg-skin-' . get_post_meta(get_the_ID(), 'eltd_masonry_gallery_simple_content_background_skin', true);
		}
	...
	public function getItemImage($params){
		...
		if(get_post_meta($id, 'eltd_masonry_gallery_item_image', true) !== '') {
			$image_url = get_post_meta($id, 'eltd_masonry_gallery_item_image', true);
			$image_id = findme_elated_get_attachment_id_from_url($image_url);
	...
	public function masonryGalleryCategoryAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				...
				$data['label'] = ( ( strlen( $value['masonry_gallery_category_title'] ) > 0 ) ? esc_html__( 'Category', 'eltd-core' ) . ': ' . $value['masonry_gallery_category_title'] : '' );
				$results[]     = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( is_object( $masonry_gallery_category ) ) {
				...
				if ( ! empty( $masonry_gallery_category_title ) ) {
					$masonry_gallery_category_title_display = esc_html__( 'Category', 'eltd-core' ) . ': ' . $masonry_gallery_category_title;
				}
