...
class ElatedfCoreImport {
	...
	public function import_options($file){
		$options = $this->file_options($file);
		$result = update_option( 'eltd_options_findme', $options);
	...
	function eltd_admin_import() {
		if (eltd_core_theme_installed()) {
			...
			$this->pagehook = add_submenu_page(
				...
				array($findme_elated_Framework->getSkin(), 'renderImport')
			);

			add_action('admin_print_scripts-'.$this->pagehook, 'findme_elated_enqueue_admin_scripts');
			add_action('admin_print_styles-'.$this->pagehook, 'findme_elated_enqueue_admin_styles');
		}
	}
}